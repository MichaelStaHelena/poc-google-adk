...
def get_fun_fact(language: str) -> dict:
    ...
    resp = client.models.generate_content(
        ...
        contents=f"One short, true fun fact about the {language} programming language.",
    )
...
gpt_analyst = Agent(
    ...
    instruction="Give a short, balanced pros and cons list (3 each) for the programming language under discussion",
...
claude_coder = Agent(
    ...
    instruction="Write one short, idiomatic, elegant code snippet (under 20 lines) in the language under discussion, "
                "then one sentence on why it reads well.",
    disallow_transfer_to_parent=True,
    disallow_transfer_to_peers=True,
)
...
root_agent = Agent(
    model="gemini-2.5-flash",
    name="fact_agent",
    instruction="You share fun facts about programming languages. "
                "Use get_fun_fact when the user asks for a fact. "
                "When the user asks for pros and cons, delegate to gpt_analyst. "
                "When the user asks for a code snippet, delegate to claude_coder.",
    tools=[get_fun_fact],
